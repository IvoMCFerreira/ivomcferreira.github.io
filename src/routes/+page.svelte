...
<style>
  ...
  * { font-family: 'Poppins', sans-serif; margin:0; padding:0; box-sizing:border-box; }
  ...
  @media (hover: hover) {
    ...
    .featured-card:hover .image-wrapper img:not(.gif) { opacity:0; }
  }
  ...
  @media (max-width: 768px) {
    ...
    .featured-card.mobile-card { width: 100%; margin-bottom: 1rem; }
    .other-grid { grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 1rem; }
    .card.mobile-card { min-height: auto; }
    .project-content.hidden-mobile { display: none; }

    .image-wrapper.clickable { cursor: pointer; position: relative; }
